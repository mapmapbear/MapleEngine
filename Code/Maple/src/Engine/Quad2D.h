...
namespace maple
{
	...
	class MAPLE_EXPORT Quad2D
	{
	  public:
		...
		inline auto &getTexCoords() const
		{
			return texCoords;
		}
		...
		inline auto setColor(const glm::vec4 &c)
		{
			color = c;
		}

		inline auto setOffset(const glm::vec2 &c)
		{
			offset = c;
		}

		inline auto setTexture(const std::shared_ptr<Texture2D> &texture)
		{
		...
		{
			return this != &nullQuad;
		}

		static Quad2D nullQuad;
		inline auto & getPivot() const
		{
			return pivot;
		}
		inline auto setPivot(const glm::vec2 &val)
		{
			pivot = val;
		}
