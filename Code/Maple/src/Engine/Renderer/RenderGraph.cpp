...
namespace maple
{
	...
	auto RenderGraph::init(uint32_t width, uint32_t height) -> void
	{
		...
		rsmData      = new ReflectiveShadowData();
		{
	...
	auto RenderGraph::onImGui() -> void
	{
				...
				if (ImGui::MenuItem(renderOutputMode(i).c_str(), "", forwardData->renderMode == i, true))
				{
					...
					switch (i)
					{
						...
						case 14:
							...
							break;
						case 15:
							descriptorSet->setTexture("uOutputSampler", rsmData->fluxTexture);
							break;
					}
				}
			}
		...
		if (rsmData->fluxTexture)
		{
			ImGuiHelper::image(rsmData->fluxTexture.get(), {64, 64});
	...
	auto RenderGraph::executeRSMPass() -> void
	{
		PROFILE_FUNCTION();
		if (!rsmData->enable)
			return;
