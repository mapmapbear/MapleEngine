...
namespace maple
{
	class MAPLE_EXPORT MapleMonoClass final
	{
		struct MethodId
		{
			...
			struct Equals
			{
				inline auto operator()(const MethodId &a, const MethodId &b) const
				{
	  ...
	  public:
		...
		inline auto &getNamespace() const
		{
			return nameSpace;
		}
		inline auto &getTypeName() const
		{
			return typeName;
		}
		inline auto &getFullName() const
		{
			return fullName;
		}
		...
		inline auto getInternalClass() const
		{
			return clazz;
		}
