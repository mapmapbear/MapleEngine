...
namespace maple
{
	...
	class MAPLE_EXPORT MonoScript final
	{
	  public:
		...
		inline auto getClassName() const
		{
			return className;
		}
		inline auto getClassNameInEditor() const
		{
			return classNameInEditor;
		}
		auto loadFunction(const std::function<void(std::shared_ptr<MapleMonoObject>)> &callback) -> void;

		inline auto getUpdateFunc()
		{
			return updateFunc;
		}
		inline auto getStartFunc()
		{
			return startFunc;
		}
		inline auto getDestoryFunc()
		{
			return destoryFunc;
		}
		inline auto getScriptObject()
		{
			return scriptObject;
		}
