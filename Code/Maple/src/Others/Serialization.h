...
namespace maple
{
	...
	namespace Serialization
	{
		...
		inline auto bulletToGlm(const btVector3& v) { return glm::vec3(v.getX(), v.getY(), v.getZ()); }
		inline auto glmToBullet(const glm::vec3& v) { return btVector3(v.x, v.y, v.z); }
		inline auto bulletToGlm(const btQuaternion& q) { return glm::quat(q.getW(), q.getX(), q.getY(), q.getZ()); }
		inline auto glmToBullet(const glm::quat& q) { return btQuaternion(q.x, q.y, q.z, q.w); }
		inline auto glmToBullet(const glm::mat3& m) { return btMatrix3x3(m[0][0], m[1][0], m[2][0], m[0][1], m[1][1], m[2][1], m[0][2], m[1][2], m[2][2]); }
