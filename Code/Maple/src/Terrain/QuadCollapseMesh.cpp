...
namespace maple
{
	...
	auto QuadNode::isDiagonalConnected(const VertNode *vn1, const VertNode *vn2) const -> bool
	{
		...
		{
			return ((vn1 == cornerVertNodes[1] && vn2 == cornerVertNodes[3])) || ((vn1 == cornerVertNodes[3] && vn2 == cornerVertNodes[1]));
		}
		else        // TM_SW_NE
		{
			return ((vn1 == cornerVertNodes[0] && vn2 == cornerVertNodes[2])) || ((vn1 == cornerVertNodes[2] && vn2 == cornerVertNodes[0]));
		}
	}
	...
	auto QuadCollapseMesh::collapseQuad(QuadNode *quadNode, int32_t x0, int32_t y0, int32_t step) -> void
	{
		...
		if (vertNodeParentSw->hasChild(vertNodeChildLf) && vertNodeParentSw->hasChild(vertNodeChildBt) && vertNodeParentNe->hasChild(vertNodeChildRt) && vertNodeParentNe->hasChild(vertNodeChildTp))
		{
		    ...
		    vertNodeParentSe->hasChild(vertNodeChildBt) && vertNodeParentSe->hasChild(vertNodeChildRt) && vertNodeParentNw->hasChild(vertNodeChildTp) && vertNodeParentNw->hasChild(vertNodeChildLf))
		{
		    ...
		    vertNodeParentSw->hasChild(vertNodeChildLf) && vertNodeParentSw->hasChild(vertNodeChildBt))
		{
			vertNodeParentSw->addChild(vertNodeChildCt);
		}
		else if (
		    vertNodeParentSe->hasChild(vertNodeChildBt) && vertNodeParentSe->hasChild(vertNodeChildRt))
		{
			vertNodeParentSe->addChild(vertNodeChildCt);
		}
		else if (
		    vertNodeParentNe->hasChild(vertNodeChildRt) && vertNodeParentNe->hasChild(vertNodeChildTp))
		{
			vertNodeParentNe->addChild(vertNodeChildCt);
		}
		else if (
		    vertNodeParentNw->hasChild(vertNodeChildTp) && vertNodeParentNw->hasChild(vertNodeChildLf))
		{
			vertNodeParentNw->addChild(vertNodeChildCt);
		}
		else
	...
	auto QuadCollapseMesh::recursiveSetActiveMesh(QuadNode *quadNode) -> void
	{
		...
		{        // not active, but a child of active quad
			if (quadNode->triangulationMode == TM_SW_NE)
			{
				addActiveVertNode(quadNode->cornerVertNodes[0]);
				addActiveVertNode(quadNode->cornerVertNodes[1]);
