...
public:
	...
	NOTIFY_INLINE auto get_current_time_stamp()
	{
	...
	NOTIFY_INLINE auto get_elapsed_time() { 
		auto ret = last_time - this->creation_time; 
		last_time = get_current_time_stamp();
		return ret;
	}
...
public:
	...
	ImGuiToast(ImGuiToastType type, int dismiss_time = NOTIFY_DEFAULT_DISMISS)
	{
		...
		this->creation_time = get_current_time_stamp();
		this->last_time = get_current_time_stamp();
